...
public:
  ...
  {
    m_VertexArray = Gazel::VertexArray::Create();
    ...
    m_Shader = Gazel::Shader::Create("VertexPosColor", vertexSrc, fragmentSrc);
    ...
    m_FlatColorShader = Gazel::Shader::Create("FlatColor", flatColorShaderVertexSrc, flatColorShaderFragmentSrc);

    auto textureShader = m_ShaderLibrary.Load("assets/shaders/Texture.glsl");

    m_Texture = Gazel::Texture2D::Create("assets/textures/Checkerboard.png");
    m_ChernoLogoTexture = Gazel::Texture2D::Create("assets/textures/ChernoLogo.png");

    std::dynamic_pointer_cast<Gazel::OpenGLShader>(textureShader)->Bind();
    std::dynamic_pointer_cast<Gazel::OpenGLShader>(textureShader)->UploadUniformInt("u_Texture", 0);
  }
  ...
  void OnUpdate(Gazel::Timestep ts) override
  {
    ...
    Gazel::Renderer::BeginScene(m_CameraController.GetCamera());

    glm::mat4 scale = glm::scale(glm::mat4(1.0f), glm::vec3(0.1f));

    std::dynamic_pointer_cast<Gazel::OpenGLShader>(m_FlatColorShader)->Bind();
    std::dynamic_pointer_cast<Gazel::OpenGLShader>(m_FlatColorShader)->UploadUniformFloat3("u_Color", m_SquareColor);
    ...
    for (int y = 0; y < 20; y++)
    {
      for (int x = 0; x < 20; x++)
      {
        ...
        Gazel::Renderer::Submit(m_FlatColorShader, m_SquareVA, transform);
      }
    }
    ...
    m_Texture->Bind();
    Gazel::Renderer::Submit(textureShader, m_SquareVA, glm::scale(glm::mat4(1.0f), glm::vec3(1.5f)));
    m_ChernoLogoTexture->Bind();
    Gazel::Renderer::Submit(textureShader, m_SquareVA, glm::scale(glm::mat4(1.0f), glm::vec3(1.5f)));

    // Triangle
    // Gazel::Renderer::Submit(m_Shader, m_VertexArray);

    Gazel::Renderer::EndScene();
  }
